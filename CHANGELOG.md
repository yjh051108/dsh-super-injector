...
## [0.3.4] — 2026-08-21（未打 tag）

### 修复

- **设置页「插件」分区渲染空白**：客户端把组件塞进 `slots.register` 的
  `options.component` 字段，且使用宿主不识别的 `{ render() }` 原生 DOM 形态——
  组件必须作为 `slots.register(options, component)` 的第二个参数（React 函数组件）。
  症状：设置对话框导航有「插件」入口，点开后右侧整页空白（分区注册成功、组件为空）。
  修复：重写为标准 React 函数组件并作为第二参数注册；样式改为模块级一次性注入
  （带守卫的选择器需引号）；60s 轮询定时器随组件卸载清理。
