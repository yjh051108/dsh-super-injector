  ...
  safeRegister(defineTool({
    ...
    async execute() {
      ...
      try {
        ...
        writeFileSync(join(tmpDir, 'src', 'index.ts'), `import type { Context } from 'cordis'\nimport type ToolRegistry from '@deepseek-ai/dsh-tools'\nimport { defineTool } from '@deepseek-ai/dsh-tools'\nexport const name = ${JSON.stringify(TEST_PKG)}\nexport const inject = ['tools']\n// 显式声明工具服务：junction 化构建环境下 'cordis'（vendor 真实副本）与\n// '@deepseek-ai/cordis'（npm 安装副本）双模块并存，dsh-tools 的 Context 增强\n// 合并不上裸 Context（TS2339: Property 'tools' does not exist）——\n// 与注入器自身 AppContext 同一模式（见 src/index.ts 主插件）。\ntype AppContext = Context & { tools: ToolRegistry }\nexport function apply(ctx: AppContext): void {\n  ctx.effect(() => ctx.tools.register(defineTool({\n    name: 'self_test_hello',\n    description: 'self test',\n    parameters: {},\n    output: { schema: { type: 'string' }, render: (_a: unknown, v: unknown) => [{ type: 'text', text: String(v) }] },\n    async execute() { return 'hello' },\n  })), 'self-test')\n}\n`, 'utf8')
